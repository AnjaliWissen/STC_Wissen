...
const BASE_URL =
  'http://ec2-13-202-153-162.ap-south-1.compute.amazonaws.com:3000/api/escrow/kuwait';
...
export function KuwaitEscrowWidget() {
  const [escrowData, setEscrowData] = useState<KuwaitEscrowData | null>(null);
  ...
  const fetchEscrowData = async () => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to fetch Kuwait escrow data');
      ...
      <Card className="p-6 bg-gradient-to-br from-white to-gray-50/50 dark:from-gray-800 dark:to-gray-900/50">
        <CardHeader>
          ...
          <CardDescription className="flex items-center gap-2 text-red-600 dark:text-red-400">
            ...
            {error ?? 'No escrow data available'}
          </CardDescription>
        </CardHeader>
      </Card>
    );
      ...
      <CardContent className="space-y-4">
        ...
        <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-900">
          ...
          <p className="text-3xl font-bold text-blue-900 dark:text-blue-300">
            {formatAmount(escrowData.escrow.total)}{' '}
            {escrowData.escrow.currency}
          </p>
        </div>
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-semibold">
              {escrowData.escrow.lockedPercentage}%
            </span>
          </div>
        ...
        <div className="p-4 rounded-lg bg-gray-50 dark:bg-gray-900/20 border">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Total Value: {formatAmount(escrowData.tokens.totalValue)}{' '}
            {escrowData.escrow.currency}
          </p>
        </div>
      </CardContent>
